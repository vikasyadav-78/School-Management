    ...
    <div className="space-y-6 animate-fade-in text-xs text-left">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 bg-white p-6 rounded-2xl border border-zinc-200 shadow-sm space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs font-semibold text-zinc-600">
            <div className="flex justify-between border-b border-zinc-100 pb-2"><span>Title:</span><span className="font-extrabold text-zinc-800">{liveClass.title || "—"}</span></div>
            <div className="flex justify-between border-b border-zinc-100 pb-2"><span>Topic:</span><span className="font-extrabold text-zinc-800">{liveClass.topic || "—"}</span></div>
            <div className="flex justify-between border-b border-zinc-100 pb-2"><span>Subject:</span><span className="font-extrabold text-zinc-800 capitalize">{liveClass.subject || "—"}</span></div>
            <div className="flex justify-between border-b border-zinc-100 pb-2"><span>Class / Section:</span><span className="font-extrabold text-zinc-800">{liveClass.class || "—"}{liveClass.section ? ` (${liveClass.section})` : ""}</span></div>
            <div className="flex justify-between border-b border-zinc-100 pb-2"><span>Platform:</span><span className="font-extrabold text-zinc-800">{liveClass.platform_label || liveClass.platform || "—"}</span></div>
            <div className="flex justify-between border-b border-zinc-100 pb-2"><span>Scheduled At:</span><span className="font-extrabold text-zinc-800">{liveClass.scheduled_at_label || "—"}</span></div>
            <div className="flex justify-between border-b border-zinc-100 pb-2"><span>Duration:</span><span className="font-extrabold text-zinc-800">{liveClass.duration_minutes || 0} Minutes</span></div>
            <div className="flex justify-between border-b border-zinc-100 pb-2"><span>Status:</span><span className={`inline-flex px-2 py-0.5 rounded-lg border text-[8px] font-black uppercase tracking-wider ${
            ...
            }`}>{liveClass.status_label || liveClass.status}</span></div>
          </div>
        </div>
        ...
        <div className="bg-white p-6 rounded-2xl border border-zinc-200 shadow-sm flex flex-col justify-between">
          <div>
            ...
            <div className="space-y-3 font-semibold text-zinc-600">
              <div className="flex justify-between items-center border-b border-zinc-100 pb-2"><span>Joined Students:</span><span className="text-base font-extrabold text-emerald-600">{joinStats.joined}</span></div>
              <div className="flex justify-between items-center border-b border-zinc-100 pb-2"><span>Not Joined Students:</span><span className="text-base font-extrabold text-rose-600">{joinStats.not_joined}</span></div>
              <div className="flex justify-between items-center pb-2"><span>Total Students:</span><span className="text-base font-extrabold text-zinc-800">{joinStats.total_students}</span></div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-zinc-200 shadow-sm overflow-hidden">
        <div className="px-6 py-4 border-b border-zinc-100 bg-zinc-50/50 flex justify-between items-center">
          ...
          <span className="px-3 py-1 bg-zinc-100 rounded-lg text-[10px] font-bold text-zinc-500 uppercase tracking-wider">Total {attendanceList.length} Roster Students</span>
        </div>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center gap-3">
                          {student.photo ? (
                            ...
                            <div className="w-8 h-8 rounded-full bg-violet-100 flex items-center justify-center text-violet-600 font-extrabold text-xs">{(student.full_name || "S").charAt(0).toUpperCase()}</div>
                          )}
                          <div>
