...
import { getAdminOnlineMcqReportDetail } from "@/features/admin/services/admin.service";
import { toast } from "sonner";

export default function AdminOnlineMcqReportDetailPage({ params }) {
  ...
  const loadData = async () => {
    try {
      setLoading(true);
      setError(null);
      const res = await getAdminOnlineMcqReportDetail(examId);
      setData(res);
    } catch (err) {
  ...
  const leaderboardColumns = [
    { header: "Rank", accessor: "rank", render: (row, idx) => <span className="font-extrabold text-amber-600">🏆 #{idx + 1}</span> },
    {
      ...
      render: (row) => (
        <button
          onClick={() => router.push(`/admin/online-mcq/reports/${examId}/attempts/${row.id}`)}
  ...
  const attendanceColumns = [
    ...
    },
    {
      ...
      render: (row) => row.attempt?.id ? (
        <Link href={`/admin/online-mcq/reports/${examId}/attempts/${row.attempt.id}`}>
  ...
  return (
    <DashboardLayout>
      <PageHeader
        ...
        subtitle={`Class: ${exam.class} (${exam.section}) | Subject: ${exam.subject} | Status: ${exam.status}`}
        action={
      ...
      <div className="space-y-6 text-xs text-zinc-700 font-semibold">
        {/* Stat Cards Grid */}
