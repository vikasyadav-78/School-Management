...
export default function StudentAllocationPage() {
  ...
  const openSingleTransfer = (student) => {
    ...
    setModalTargetClassId(student.school_class_id || "");
  ...
  const handleTransferSubmit = async (e) => {
    ...
    try {
      ...
      if (isBulkAction) {
        ...
        await transferStudents(payload);
  ...
  const handleAssignSubmit = async (e) => {
    ...
    try {
      ...
      if (isBulkAction) {
        await bulkAssignStudents(payload);
        toast.success(`Successfully allocated ${studentIds.length} students.`);
      } else {
        await assignStudentsToSection(payload);
    ...
    <DashboardLayout>
      ...
      <div className="flex border-b border-zinc-200 gap-6 mb-6">
        <button
          ...
          className={`py-3 text-xs font-bold flex items-center gap-1.5 border-b-2 transition-all cursor-pointer ${
            activeTab === "workspace" 
          ...
          {!selectedClass ? (
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {allocationData.classes?.map((cls) => (
                  <div
                    key={cls.id}
                ...
                <div className="flex flex-wrap items-center gap-3">
                  ...
                  <div className="relative w-full sm:w-60">
                    ...
                    <input
                      ...
                      className="w-full pl-8 pr-3 py-1.5 border border-zinc-200 rounded-xl text-[10px] font-semibold outline-none focus:border-violet-500 bg-zinc-50 focus:bg-white"
                    />
                  </div>
                  ...
                  <select
                    ...
                    className="px-3 py-1.5 border border-zinc-200 rounded-xl text-[10px] font-bold text-zinc-700 bg-white outline-none cursor-pointer"
                  >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-zinc-950/50 backdrop-blur-sm animate-fade-in">
          <div className="bg-white rounded-2xl border border-zinc-200 shadow-2xl w-full max-w-sm overflow-hidden animate-scale-up text-left">
            ...
            <form onSubmit={handleAssignSubmit} className="p-6 space-y-4 text-xs font-semibold">
              ...
              {/* Class Selection (Lock to selected workspace class if bulk) */}
              <div className="space-y-1">
