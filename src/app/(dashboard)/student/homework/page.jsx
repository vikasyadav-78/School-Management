...
import {
  FaBook, FaSearch, FaClock, FaCheckCircle, FaExclamationTriangle,
  FaDownload, FaFileUpload, FaTimes, FaUser, FaStickyNote, FaGraduationCap
} from "react-icons/fa";
...
export default function StudentHomeworkPage() {
  ...
  const getStatusBadge = (h) => {
    if (h.is_overdue) {
      return (
        <span className="px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-rose-50 border border-rose-200 text-rose-700 uppercase tracking-wider flex items-center gap-1">
    ...
    switch (status) {
      ...
      default:
        return (
          <span className="px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-amber-50 border border-amber-200 text-amber-700 uppercase tracking-wider">
            Pending
          </span>
        );
    }
  ...
  return (
    <div className="space-y-6 animate-fade-in text-left w-full">
      <PageHeader
        ...
        description="Access class coursework, download reference files, and submit assignments online."
      />
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-white p-4.5 rounded-2xl border border-zinc-200 shadow-sm text-center">
          <span className="text-[11px] font-bold text-zinc-400 uppercase tracking-wider block">All Homework</span>
          <h3 className="text-2xl font-black text-zinc-800 mt-1">{summary.all}</h3>
        </div>
        <div className="bg-blue-50 border border-blue-100 p-4.5 rounded-2xl text-center">
          <span className="text-[11px] font-bold text-blue-600 uppercase tracking-wider block">Due Today</span>
          <h3 className="text-2xl font-black text-blue-700 mt-1">{summary.today}</h3>
        </div>
        <div className="bg-amber-50 border border-amber-100 p-4.5 rounded-2xl text-center">
          ...
          <h3 className="text-2xl font-black text-amber-700 mt-1">{summary.pending}</h3>
        </div>
        <div className="bg-emerald-50 border border-emerald-100 p-4.5 rounded-2xl text-center">
          ...
          <h3 className="text-2xl font-black text-emerald-700 mt-1">{summary.completed}</h3>
        </div>
      </div>
      ...
      {filteredHomework.length === 0 ? (
        <div className="p-12 bg-white rounded-2xl border border-zinc-200 shadow-sm text-center">
          <span className="text-zinc-400 font-bold uppercase tracking-wider text-xs block mb-1">No Homework Available</span>
          <span className="text-zinc-400 text-xs">There are no homework assignments matching your filter tabs or search input.</span>
        </div>
      ) : (
        <>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            {filteredHomework.slice((currentPage - 1) * pageSize, currentPage * pageSize).map((h) => (
              <div
                key={h.id}
                ...
                <div className="space-y-3">
                  <div className="flex justify-between items-start gap-3">
                    <div>
                      ...
                      <h4 className="font-bold text-zinc-900 text-sm group-hover:text-violet-600 transition-colors mt-2 line-clamp-1">
                        {h.title}
                      </h4>
                    </div>
                    {getStatusBadge(h)}
                  ...
                  <div className="space-y-1.5 pt-2 text-xs text-zinc-600 border-t border-zinc-100">
                    <div className="flex items-center gap-2">
                      ...
                      <span>Teacher: <strong className="text-zinc-800">{h.teacher || h.teacher_name || "N/A"}</strong></span>
                    </div>
                    <div className="flex items-center gap-2">
                      <FaClock className="w-3.5 h-3.5 text-zinc-400 shrink-0" />
                      <span>Due: <strong className="text-zinc-800">{h.due_date || "N/A"}</strong></span>
                    </div>
                    <div className="flex items-center gap-2">
                      <FaGraduationCap className="w-3.5 h-3.5 text-zinc-400 shrink-0" />
                      <span>Max Marks: <strong className="text-zinc-800">{h.max_marks !== null ? h.max_marks : "—"}</strong></span>
                    </div>
                  </div>
                </div>
          ...
          {filteredHomework.length > pageSize && (
                ...
                <>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 bg-zinc-50 p-3.5 rounded-xl border border-zinc-200/60">
                    <div className="space-y-0.5">
                      ...
                      <span className="font-bold text-zinc-800 text-xs block">{activeDetail.teacher}</span>
                    </div>
                    <div className="space-y-0.5">
                      <span className="text-[10px] font-bold text-zinc-400 uppercase tracking-wider block">Max Marks</span>
                      <span className="font-bold text-zinc-800 text-xs block">{activeDetail.max_marks !== null ? `${activeDetail.max_marks} Marks` : "—"}</span>
                    </div>
                    <div className="space-y-0.5">
                      <span className="text-[10px] font-bold text-zinc-400 uppercase tracking-wider block">Assigned Date</span>
                      <span className="font-bold text-zinc-800 text-xs block font-mono">{activeDetail.assigned_date}</span>
                    </div>
                    <div className="space-y-0.5">
                      <span className="text-[10px] font-bold text-zinc-400 uppercase tracking-wider block">Due Date</span>
                      <span className="font-bold text-zinc-800 text-xs block font-mono">{activeDetail.due_date}</span>
                    </div>
                  </div>
                  ...
                  {activeDetail.is_submitted ? (
                    <div className="border-t border-zinc-100 pt-4 space-y-3">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 bg-zinc-50 p-3.5 rounded-xl border border-zinc-200/60">
                        <div className="space-y-0.5">
                          <span className="text-[10px] font-bold text-zinc-400 uppercase tracking-wider block">Submitted Date</span>
                          <span className="font-bold text-zinc-800 text-xs">{activeDetail.submission?.submitted_at_label || "N/A"}</span>
                        </div>
                      ...
                      {activeDetail.submission?.content && (
                        <div className="space-y-1">
                          <span className="text-[10px] font-bold text-zinc-400 uppercase tracking-wider block">Submitted Content</span>
                          <p className="p-3 border border-zinc-200/60 rounded-xl text-zinc-700 bg-white leading-relaxed font-semibold">
                            {activeDetail.submission.content}
                          </p>
                        </div>
                      )}
                      ...
                      {activeDetail.submission?.file_url && (
                        <div className="space-y-1">
                          <span className="text-[10px] font-bold text-zinc-400 uppercase tracking-wider block">Submitted File Attachment</span>
                          <a
                      ...
                      {activeDetail.submission?.feedback && (
                        <div className="p-3.5 bg-amber-50/60 border border-amber-200/60 rounded-xl space-y-1">
                          ...
                          <p className="text-zinc-800 font-bold text-xs">{activeDetail.submission.feedback}</p>
                        </div>
                      )}
                    </div>
                  ) : (
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-zinc-950/45 backdrop-blur-sm animate-fade-in">
          <div className="bg-white rounded-2xl border border-zinc-200 shadow-2xl w-full max-w-lg overflow-hidden animate-scale-up">
            ...
            <form onSubmit={handleSubmit}>
              <div className="p-6 space-y-4 text-xs text-left">
                <div className="space-y-1.5">
                  ...
                  <textarea
                    ...
                    className="w-full p-3 border border-zinc-200 rounded-xl outline-none focus:border-violet-500 bg-zinc-50 focus:bg-white transition-all font-semibold text-zinc-800"
                  />
                  <div className="flex justify-end text-[10px] text-zinc-400 font-semibold">
              ...
              <div className="px-6 py-4 bg-zinc-50 border-t border-zinc-100 flex justify-end gap-2.5">
                <button
                  type="button"
                  onClick={() => setIsSubmitOpen(false)}
                  className="px-4 py-2 bg-zinc-200 hover:bg-zinc-300 text-zinc-700 font-bold rounded-xl transition-all cursor-pointer"
