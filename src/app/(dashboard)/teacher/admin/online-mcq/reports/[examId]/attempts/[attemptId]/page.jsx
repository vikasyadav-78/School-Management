...
export default function TeacherOnlineMcqAttemptDetailPage({ params }) {
  ...
  const loadData = async () => {
    try {
      setLoading(true);
      setError(null);
      const res = await getTeacherOnlineMcqAttempt(examId, attemptId);
      setData(res);
    } catch (err) {
  ...
  const handleDownloadPdf = async () => {
    try {
      setDownloading(true);
      const blob = await getTeacherOnlineMcqAttemptPdf(examId, attemptId);
  ...
  return (
    <div className="space-y-6 text-xs text-zinc-700 font-semibold text-left animate-fade-in">
      <PageHeader
        title={`Exam Attempt — ${attempt?.student?.full_name || "Student"}`}
        subtitle={`Exam: ${exam.title || "N/A"} | Submission summary & answer sheet.`}
        action={
