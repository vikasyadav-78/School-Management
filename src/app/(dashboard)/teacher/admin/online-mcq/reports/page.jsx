...
import { getTeacherOnlineMcqReports } from "@/features/teachers/services/teacher.service";
import { toast } from "sonner";

export default function TeacherOnlineMcqReportsListPage() {
  ...
  const loadData = async () => {
    try {
      setLoading(true);
      setError(null);
      const res = await getTeacherOnlineMcqReports();
    ...
    <div className="space-y-6 text-xs text-left animate-fade-in">
      <PageHeader
        ...
        action={
          <div className="flex gap-2">
            <Link href="/teacher/admin/online-mcq">
              <Button variant="outline" size="sm">
                <FaArrowLeft className="mr-1.5" /> Back to MCQ
              </Button>
            </Link>
            <Link href="/teacher/admin/online-mcq/reports/pass-fail">
      ...
      <div className="space-y-4">
        ...
        <DataTable
          ...
          loading={loading}
          onView={(row) => router.push(`/teacher/admin/online-mcq/reports/${row.id}`)}
