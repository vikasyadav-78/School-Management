...
import { changeTeacherPassword } from "@/features/teachers/redux/teacherThunk";
...
export default function TeacherChangePasswordPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await dispatch(changeTeacherPassword(payload)).unwrap();
  ...
  return (
    <div className="space-y-6 animate-fade-in text-xs max-w-md mx-auto">
      ...
      <div className="bg-white p-6 rounded-2xl border border-zinc-200/80 shadow-sm space-y-6">
        <div className="flex items-center gap-2 text-zinc-700 font-extrabold text-xs uppercase tracking-wider pb-3 border-b border-zinc-150">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="pt-2">
            <Button
              type="submit"
              disabled={submitting}
              className="w-full bg-violet-600 hover:bg-violet-750 text-white font-bold py-2.5 rounded-lg flex items-center justify-center gap-2 text-xs shadow-sm shadow-violet-600/10"
            >
              {submitting ? (
