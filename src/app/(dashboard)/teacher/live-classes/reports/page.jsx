...
export default function TeacherLiveClassReportsPage() {
  ...
  const attendanceSummary = reportSummary.attendance_summary || {};
      ...
      <div className="grid grid-cols-2 lg:grid-cols-6 gap-4">
        ...
        <div className="bg-white p-4 rounded-2xl border border-zinc-200 shadow-sm flex flex-col justify-between">
          ...
          <div className="flex items-baseline justify-between mt-2">
            <span className="text-xl font-extrabold text-zinc-800">{reportSummary.total_classes || 0}</span>
        ...
        <div className="bg-white p-4 rounded-2xl border border-zinc-200 shadow-sm flex flex-col justify-between">
          ...
          <div className="flex items-baseline justify-between mt-2">
            <span className="text-xl font-extrabold text-zinc-800">{reportSummary.recordings_count || 0}</span>
        ...
        <div className="bg-white p-4 rounded-2xl border border-zinc-200 shadow-sm flex flex-col justify-between">
          ...
          <div className="flex items-baseline justify-between mt-2">
            <span className="text-xl font-extrabold text-emerald-600">{attendanceSummary.present || 0}</span>
        ...
        <div className="bg-white p-4 rounded-2xl border border-zinc-200 shadow-sm flex flex-col justify-between">
          ...
          <div className="flex items-baseline justify-between mt-2">
            <span className="text-xl font-extrabold text-amber-600">{attendanceSummary.late || 0}</span>
        ...
        <div className="bg-white p-4 rounded-2xl border border-zinc-200 shadow-sm flex flex-col justify-between">
          ...
          <div className="flex items-baseline justify-between mt-2">
            <span className="text-xl font-extrabold text-rose-600">{attendanceSummary.absent || 0}</span>
        ...
        <div className="bg-white p-4 rounded-2xl border border-zinc-200 shadow-sm flex flex-col justify-between">
          ...
          <div className="flex items-baseline justify-between mt-2">
            <span className="text-xl font-extrabold text-purple-600">{attendanceSummary.early_exit || 0}</span>
        ...
        <div className="space-y-6">
            ...
            {liveClassesList.map((lc) => {
              ...
              return (
                <div key={lc.id} className="bg-white border border-zinc-200 rounded-2xl p-5 shadow-sm flex flex-col justify-between hover:shadow-md transition-all">
                  <div>
                    <div className="flex items-center justify-between mb-3">
                      <span className={`inline-flex px-2.5 py-0.5 rounded-lg border text-[8px] font-black uppercase tracking-wider ${
                        lc.status === "completed" ? "bg-emerald-50 border-emerald-100 text-emerald-600" :
                        lc.status === "cancelled" ? "bg-rose-50 border-rose-100 text-rose-600" :
                        "bg-violet-50 border-violet-100 text-violet-600"
                      }`}>
                        {lc.platform_label || lc.platform || "LIVE"} • {lc.status_label || lc.status}
                      </span>
                    </div>
                  ...
                  <button
                    onClick={() => router.push(`/teacher/live-classes/reports/${lc.id}`)}
          ...
          {lastPage > 1 && (
            <div className="flex items-center justify-between border-t border-zinc-100 pt-4 text-xs font-bold text-zinc-600">
              <span>Showing {liveClassesList.length} of {totalCount} records</span>
