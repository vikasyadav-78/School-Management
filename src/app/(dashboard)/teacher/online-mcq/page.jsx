...
import {
  ...
  getTeacherOnlineMcqExamDetail,
  addTeacherOnlineMcqQuestion,
  deleteTeacherOnlineMcqQuestion,
...
export default function TeacherOnlineMcqPage() {
  ...
  // Filters
  const [statusFilter, setStatusFilter] = useState("all"); // "all" | "draft" | "published" | "expired"
  const [classFilter, setClassFilter] = useState("all");

  // Selected Exam Inspector
  const [activeExam, setActiveExam] = useState(null);
  const [isDetailModalOpen, setIsDetailModalOpen] = useState(false);

  // Create Exam Modal State (No Predefined Settings)
  ...
  const [questionType, setQuestionType] = useState("mcq_single"); // "mcq_single" | "fill_blank"
  ...
  const loadMeta = async () => {
    try {
      setLoading(true);
      const metaData = await getTeacherOnlineMcqMeta();
      setMeta(metaData.meta || metaData.data || metaData);

      const listData = await getTeacherOnlineMcq();
  ...
  const refreshList = async () => {
    try {
      ...
      const listData = await getTeacherOnlineMcq(params);
      setExams(listData.exams || listData.data || (Array.isArray(listData) ? listData : []));
    } catch (err) {
      console.error(err);
    } finally {
  ...
  // Open Inspector / Question Bank
  const handleOpenDetail = async (exam) => {
    try {
      const detailed = await getTeacherOnlineMcqExamDetail(exam.id);
  ...
  // Reset Exam Form (No predefined defaults)
  const resetExamForm = () => {
    setExamTitle("");
    setSubjectId("");
    setClassId("");
  ...
  // Create Exam Draft Handler with Instant Question Editor Transition
  const handleSaveExam = async (e) => {
    ...
    try {
      ...
      const res = await addTeacherOnlineMcqExam(payload);
      toast.success("Exam draft created successfully!");
      setIsExamModalOpen(false);

      // Instant Transition into Question Editor Inspector!
      const newExamObj = res.exam || res.data || res;
      const newExamId = newExamObj?.id;

      if (newExamId) {
        try {
          const detailed = await getTeacherOnlineMcqExamDetail(newExamId);
          setActiveExam(detailed.exam || detailed.data || detailed || newExamObj);
        } catch (detailErr) {
          setActiveExam(newExamObj);
        }
        setIsDetailModalOpen(true);
        handleOpenAddQuestion(); // Automatically open Add Question Modal for immediate entry!
      }
  ...
  const handlePublishExam = async (examId) => {
    ...
    try {
      await publishTeacherOnlineMcqExam(examId);
      toast.success("Exam published successfully!");
      if (activeExam?.id === examId) {
        const detailed = await getTeacherOnlineMcqExamDetail(examId);
  ...
  const handleDeleteExam = async (examId) => {
    ...
    if (!isConfirmed) return;
    try {
      await deleteTeacherOnlineMcqExam(examId);
  ...
  const handleSaveQuestion = async (e, keepModalOpen = false) => {
    ...
    try {
      ...
      await addTeacherOnlineMcqQuestion(activeExam.id, payload);
      toast.success("Question added! Ready for next question.");

      if (activeExam?.id) {
        const detailed = await getTeacherOnlineMcqExamDetail(activeExam.id);
        setActiveExam(detailed.exam || detailed.data || detailed);
      }
  ...
  const handleDeleteQuestion = async (questionId) => {
    ...
    if (!isConfirmed) return;
    try {
      await deleteTeacherOnlineMcqQuestion(activeExam.id, questionId);
    ...
    <div className="space-y-6 animate-fade-in text-xs text-left">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <button
          ...
          className="px-4 py-2.5 bg-violet-600 hover:bg-violet-700 text-white rounded-xl font-bold flex items-center justify-center gap-1.5 transition-all self-start sm:self-auto cursor-pointer shadow-sm text-xs"
        >
              ...
              <div key={e.id} className="bg-white border border-zinc-200 rounded-2xl p-5 shadow-sm relative flex flex-col justify-between hover:shadow-md transition-all">
                <div>
                  ...
                  <span className="text-[10px] text-zinc-400 font-bold uppercase tracking-wider block mb-3">
                    Subject: <span className="text-zinc-700 capitalize">{e.subject}</span> • Class: <span className="text-zinc-700">{e.class}{e.stream ? ` (${e.stream})` : ""}</span>
                  </span>
                  ...
                  <div className="space-y-1.5 text-[10px] font-semibold text-zinc-500 mb-4 bg-zinc-50 p-3 rounded-xl border border-zinc-100">
                    <div className="flex justify-between">
                      <span>Questions:</span>
                      <span className="font-bold text-zinc-800">{e.questions_count || e.questions?.length || 0}</span>
                    </div>
                    <div className="flex justify-between">
                      <span>Duration:</span>
                      <span className="font-bold text-zinc-800">{e.duration_minutes || 60} Mins</span>
                    </div>
                    <div className="flex justify-between">
                      <span>Starts At:</span>
                      <span className="font-bold text-zinc-800">{e.starts_at_label || formatDateTime(e.starts_at)}</span>
                    </div>
                    <div className="flex justify-between">
                      <span>Ends At:</span>
                      <span className="font-bold text-zinc-800">{e.ends_at_label || formatDateTime(e.ends_at)}</span>
                    </div>
                    <div className="flex justify-between">
                      <span>Negative Marking:</span>
                      <span className="font-bold text-zinc-800">{e.negative_marking ? `Yes (-${e.negative_marks})` : "No"}</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex gap-2 pt-2 border-t border-zinc-100">
                  <button
                    ...
                    className="flex-1 py-2 bg-zinc-100 hover:bg-violet-50 hover:text-violet-600 text-zinc-700 font-bold rounded-xl transition-all flex items-center justify-center gap-1.5 cursor-pointer text-xs"
                  >
                  ...
                  {!isPublished && (
                    <button
                      onClick={() => handlePublishExam(e.id)}
                      className="px-3.5 py-2 bg-emerald-600 hover:bg-emerald-700 text-white font-bold rounded-xl transition-all flex items-center gap-1 cursor-pointer text-xs shadow-sm"
                    >
      ...
      {/* Create Exam Draft Modal (Zero Predefined Settings) */}
      {isExamModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-zinc-950/45 backdrop-blur-sm animate-fade-in">
          <div className="bg-white rounded-2xl border border-zinc-200 shadow-2xl w-full max-w-lg overflow-hidden animate-scale-up text-left flex flex-col">
            ...
            <form onSubmit={handleSaveExam} className="p-6 space-y-4 max-h-[78vh] overflow-y-auto custom-scrollbar">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  ...
                  <select
                    ...
                    className="w-full px-3 py-2 border border-zinc-200 rounded-xl bg-zinc-50 outline-none text-xs font-bold text-zinc-700 cursor-pointer"
                  >
              ...
              {/* Required Exam Security & Configuration Options */}
              <div className="p-4 bg-zinc-50 rounded-2xl border border-zinc-150 space-y-3">
                <span className="text-[10px] font-extrabold text-zinc-700 uppercase tracking-wider block border-b border-zinc-200/80 pb-1.5">
                  Exam Rules & Security Options
                </span>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-zinc-950/45 backdrop-blur-sm animate-fade-in">
            <div className="bg-white rounded-2xl border border-zinc-200 shadow-2xl w-full max-w-3xl overflow-hidden animate-scale-up text-left flex flex-col">
              ...
              <div className="p-6 space-y-6 max-h-[75vh] overflow-y-auto custom-scrollbar">
                {/* Professional Workflow Alert Card */}
                ...
                <div className="bg-zinc-50 p-4 rounded-2xl border border-zinc-150 grid grid-cols-2 sm:grid-cols-4 gap-3 text-center">
                  <div className="p-2 bg-white rounded-xl border border-zinc-100 shadow-2xs">
                    <span className="text-[9px] font-bold text-zinc-400 uppercase tracking-wider block">Questions</span>
                    <span className="text-base font-black text-zinc-800">{questionsList.length}</span>
                  </div>

                  <div className="p-2 bg-white rounded-xl border border-zinc-100 shadow-2xs">
                    <span className="text-[9px] font-bold text-zinc-400 uppercase tracking-wider block">Total Marks</span>
                    <span className="text-base font-black text-violet-600">{totalMarksSum || activeExam.total_marks || 0}</span>
                  </div>

                  <div className="p-2 bg-white rounded-xl border border-zinc-100 shadow-2xs">
                    <span className="text-[9px] font-bold text-zinc-400 uppercase tracking-wider block">MCQ / Blanks</span>
                    <span className="text-xs font-black text-zinc-800">{mcqCount} MCQ / {fillBlankCount} Blank</span>
                  </div>

                  <div className="p-2 bg-white rounded-xl border border-zinc-100 shadow-2xs">
                    <span className="text-[9px] font-bold text-zinc-400 uppercase tracking-wider block">Status</span>
                    <span className={`text-xs font-black uppercase ${isPublished ? "text-emerald-600" : "text-amber-600"}`}>
                      {isPublished ? "Published" : "Draft"}
                    </span>
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between border-b border-zinc-100 pb-2">
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      className="px-3 py-1.5 bg-violet-600 hover:bg-violet-700 text-white font-bold rounded-xl text-[10px] uppercase tracking-wider flex items-center gap-1 cursor-pointer shadow-xs"
                    >
                    ...
                    {!isPublished && (
                      <button
                        onClick={() => handlePublishExam(activeExam.id)}
                        className="px-3 py-1.5 bg-emerald-600 hover:bg-emerald-700 text-white font-bold rounded-xl text-[10px] uppercase tracking-wider flex items-center gap-1 cursor-pointer shadow-xs"
                      >
          ...
          <div className="bg-white rounded-2xl border border-zinc-200 shadow-2xl w-full max-w-md overflow-hidden animate-scale-up text-left flex flex-col">
            ...
            <form onSubmit={(e) => handleSaveQuestion(e, false)} className="p-6 space-y-4 max-h-[75vh] overflow-y-auto custom-scrollbar">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1">
                  <label className="text-[10px] font-bold text-zinc-500 uppercase tracking-wider block">Option A</label>
                  <input type="text" value={optionA} onChange={(e) => setOptionA(e.target.value)} className="w-full px-3 py-1.5 border border-zinc-200 rounded-xl outline-none text-black font-semibold" />
                </div>
                <div className="space-y-1">
                  <label className="text-[10px] font-bold text-zinc-500 uppercase tracking-wider block">Option B</label>
                  <input type="text" value={optionB} onChange={(e) => setOptionB(e.target.value)} className="w-full px-3 py-1.5 border border-zinc-200 rounded-xl outline-none text-black font-semibold" />
                </div>
                <div className="space-y-1">
                  <label className="text-[10px] font-bold text-zinc-500 uppercase tracking-wider block">Option C</label>
                  <input type="text" value={optionC} onChange={(e) => setOptionC(e.target.value)} className="w-full px-3 py-1.5 border border-zinc-200 rounded-xl outline-none text-black font-semibold" />
                </div>
                <div className="space-y-1">
                  <label className="text-[10px] font-bold text-zinc-500 uppercase tracking-wider block">Option D</label>
                  <input type="text" value={optionD} onChange={(e) => setOptionD(e.target.value)} className="w-full px-3 py-1.5 border border-zinc-200 rounded-xl outline-none text-black font-semibold" />
                </div>
              </div>
