...
import { FaUserCircle, FaSearch, FaCheckCircle } from "react-icons/fa";
import Link from "next/link";
import Pagination from "@/components/ui/Pagination";

export default function SalaryHistoryTable({ records = [], onViewReceipt }) {
          ...
          <table className="w-full text-left border-collapse border-spacing-0">
            <thead>
              <tr className="border-b border-zinc-100 bg-zinc-50 text-[10px] font-bold text-zinc-400 uppercase tracking-wider">
                <th className="px-6 py-4 whitespace-nowrap">Voucher No</th>
                <th className="px-6 py-4 whitespace-nowrap">Teacher</th>
                <th className="px-6 py-4 text-center whitespace-nowrap">Pay Month</th>
                <th className="px-6 py-4 text-right whitespace-nowrap">Amount Paid</th>
                <th className="px-6 py-4 text-center whitespace-nowrap">Payment Date</th>
            ...
            <tbody className="divide-y divide-zinc-100 text-xs font-medium text-zinc-600">
              {paginatedRecords.map((rec) => (
                <tr key={rec.id} className="hover:bg-zinc-50/50 transition-colors">
                  <td className="px-6 py-3.5 font-bold text-zinc-800 whitespace-nowrap">{rec.id}</td>
                  ...
                  <td className="px-6 py-3.5 text-center font-bold text-zinc-700 whitespace-nowrap">{rec.month}</td>
                  <td className="px-6 py-3.5 text-right font-extrabold text-emerald-600 whitespace-nowrap">₹{(rec.finalSalary || 0).toLocaleString()}</td>
                  <td className="px-6 py-3.5 text-center text-zinc-400 font-semibold whitespace-nowrap">{rec.paymentDate}</td>
                  <td className="px-6 py-3.5 text-center text-zinc-600 font-semibold whitespace-nowrap">{rec.paymentMethod}</td>
              ...
              {filteredRecords.length === 0 && (
                <tr>
                  <td colSpan="7" className="text-center py-12 text-zinc-400 font-medium">
                    No historical salary payments found.
                  </td>
                </tr>
              )}
            </tbody>
