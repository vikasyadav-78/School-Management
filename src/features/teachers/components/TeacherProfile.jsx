...
import { 
  ...
  FaMapMarkerAlt, FaFileAlt, FaQrcode, FaPrint, FaIdCard, FaSignInAlt, FaEdit, FaArrowLeft
} from "react-icons/fa";
  ...
  return (
    <div className="space-y-6 animate-fade-in text-xs text-left text-zinc-600">
      ...
      <div className="flex flex-wrap gap-2 pt-1">
        ...
        <button
          onClick={() => window.open(`https://erp.trishpay.in/school-admin/teachers/${teacher.id}/id-card?theme=classic`, '_blank')}
          className="px-4 py-2 text-xs font-bold text-zinc-600 bg-white border border-zinc-200 hover:bg-zinc-50 rounded-xl transition-all cursor-pointer shadow-sm flex items-center gap-1.5"
        >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-6">
          <div className="bg-white p-6 rounded-2xl border border-zinc-200 shadow-sm text-center space-y-4">
            <h4 className="text-[10px] font-extrabold text-zinc-800 block uppercase tracking-wider border-b border-zinc-100 pb-2">Quick QR</h4>
            <div className="w-36 h-36 bg-zinc-50 border border-zinc-100 rounded-xl flex items-center justify-center mx-auto shadow-inner overflow-hidden p-2">
              <img 
            ...
            <p className="text-[10px] text-zinc-400 font-extrabold tracking-wider">{teacher.employee_id || "—"}</p>
            <button
              onClick={() => window.open(`https://erp.trishpay.in/school-admin/teachers/${teacher.id}/id-card?theme=classic`, '_blank')}
              className="w-full py-2.5 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-xl transition-all cursor-pointer shadow-sm text-xs flex items-center justify-center gap-1.5"
            >
